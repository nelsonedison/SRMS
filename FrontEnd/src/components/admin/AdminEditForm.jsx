...
const AdminEditForm = ({ isOpen, onClose, onSuccess, form }) => {
  ...
  const { updateForm, loading, toast, closeToast } = useForm();
  ...
  useEffect(() => {
    if (isOpen && form) {
      setFormData({
        ...
        approval_workflow: form.approval_workflow || 'tutor_hod_principal',
  ...
  const handleClose = () => {
    setFormData({ title: '', description: '', department: '', approval_workflow: 'tutor_hod_principal', is_active: true, fields: [] });
    onClose();
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Edit Form" size="xl">
