...
import { Eye, Check, X, User, CheckCircle } from 'lucide-react';
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500 mb-3">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        submission.status.includes('pending') ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      {submission.form_approval_workflow !== 'hod_only' && (
                        <>
                          <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${
                            ...
                            <span>Tutor</span>
                            {submission.tutor_reviewed_at && <CheckCircle className="w-3 h-3" />}
                          </div>
                      ...
                      {['hod_only', 'tutor_hod', 'tutor_hod_principal'].includes(submission.form_approval_workflow) && (
                        <>
                          <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${
                            ...
                            <User className="w-3 h-3" />
                            <span>HOD</span>
                            {submission.hod_reviewed_at && <CheckCircle className="w-3 h-3" />}
                          </div>
                      ...
                      {submission.form_approval_workflow === 'tutor_hod_principal' && (
                        <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${
                          ...
                          <User className="w-3 h-3" />
                          <span>Principal</span>
                          {submission.principal_reviewed_at && <CheckCircle className="w-3 h-3" />}
                        </div>
                      )}
                    </div>
                    {submission.comments && (
