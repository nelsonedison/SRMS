...
const PrincipalForms = () => {
  ...
  const [newForm, setNewForm] = useState({
    ...
    description: '',
    department: '',
    approval_workflow: 'tutor_hod_principal',
    fields: []
  });
  ...
  const handleCreateForm = async () => {
    try {
      ...
      setNewForm({ title: '', description: '', department: '', approval_workflow: 'tutor_hod_principal', fields: [] });
      fetchForms();
    } catch (error) {
          ...
          <div key={form.id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="p-6">
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <FileText className="w-4 h-4" />
                  <span>Workflow: {getWorkflowDisplay(form.approval_workflow)}</span>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <select
                  value={newForm.approval_workflow}
                  onChange={(e) => setNewForm({...newForm, approval_workflow: e.target.value})}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                >
                  <option value="tutor_only">Tutor Only</option>
                  <option value="hod_only">HOD Only</option>
                  <option value="tutor_hod">Tutor → HOD</option>
                  <option value="tutor_hod_principal">Tutor → HOD → Principal</option>
                </select>
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-semibold text-gray-900">Form Details</h3>
                <button
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  <h4 className="font-semibold text-gray-900 mb-3">Form Information</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-gray-900">{selectedForm.department_name || 'All Departments'}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-600">Approval Workflow</label>
                      <p className="text-gray-900">{getWorkflowDisplay(selectedForm.approval_workflow)}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-600">Status</label>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        selectedForm.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
                ...
                {/* Form Fields */}
                <div>
                  <h4 className="font-semibold text-gray-900 mb-4">Form Fields ({selectedForm.fields?.length || 0})</h4>
                        ...
                        {field.options && field.options.length > 0 && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {field.options.map((option, optIndex) => (
                                <span key={optIndex} className="px-2 py-1 bg-white border rounded text-xs text-gray-700">
                                  {option}
                                </span>
                              ))}
                            </div>
                ...
                {/* Form Meta */}
                <div className="bg-gray-50 rounded-xl p-4">
                  <h4 className="font-semibold text-gray-900 mb-3">Form Metadata</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <label className="font-medium text-gray-600">Created By</label>
                      <p className="text-gray-900">{selectedForm.created_by_name} ({selectedForm.created_by_type})</p>
                    </div>
                    <div>
                      <label className="font-medium text-gray-600">Created At</label>
                      <p className="text-gray-900">{new Date(selectedForm.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </div>
