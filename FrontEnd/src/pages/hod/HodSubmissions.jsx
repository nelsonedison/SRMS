...
import { getWorkflowDisplay } from '../../utils/workflowUtils';
              ...
              <div key={submission.id} className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-6 border border-orange-200">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {/* Workflow Info */}
                    <div className="mb-3">
                      <span className="text-sm text-gray-600">Workflow: {getWorkflowDisplay(submission.form_approval_workflow)}</span>
                    </div>

                    {/* Approval Timeline */}
                    <div className="flex items-center gap-2 mb-4">
                      {/* Tutor Step - Show for all workflows except hod_only */}
                      {submission.form_approval_workflow !== 'hod_only' && (
                        <>
                          <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${submission.tutor_reviewed_at ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                            }`}>
                            <User className="w-3 h-3" />
                            <span>Tutor</span>
                            {submission.tutor_reviewed_at ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                          </div>
                          {submission.form_approval_workflow !== 'tutor_only' && <span className="text-gray-300">→</span>}
                        </>
                      )}

                      {/* HOD Step - Show for workflows that include HOD */}
                      {['hod_only', 'tutor_hod', 'tutor_hod_principal'].includes(submission.form_approval_workflow) && (
                        <>
                          <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${submission.hod_reviewed_at ? 'bg-green-100 text-green-700' :
                              (submission.form_approval_workflow === 'hod_only' || submission.tutor_reviewed_at) ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'
                            }`}>
                            <User className="w-3 h-3" />
                            <span>HOD</span>
                            {submission.hod_reviewed_at ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                          </div>
                          {submission.form_approval_workflow === 'tutor_hod_principal' && <span className="text-gray-300">→</span>}
                        </>
                      )}

                      {/* Principal Step - Show only for tutor_hod_principal workflow */}
                      {submission.form_approval_workflow === 'tutor_hod_principal' && (
                        <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs ${submission.principal_reviewed_at ? 'bg-green-100 text-green-700' :
                            submission.hod_reviewed_at ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-500'
                          }`}>
                          <User className="w-3 h-3" />
                          <span>Principal</span>
                          {submission.principal_reviewed_at ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                        </div>
                      )}
                    </div>

                    {/* Tutor Comments */}
                    {submission.tutor_comments && submission.tutor_comments.trim() && submission.form_approval_workflow !== 'hod_only' && (
