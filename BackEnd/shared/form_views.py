...
def submit_form(request, form_id):
    ...
    if serializer.is_valid():
        ...
        # Set initial status and workflow based on form
        submission.status = submission.get_initial_status()
        submission.approval_workflow = form.approval_workflow
        submission.save()
...
def review_submission(request, submission_id):
    ...
    # Check if user can review this submission based on workflow
    workflow = submission.approval_workflow
    current_status = submission.status
    
    # Validate reviewer based on current status and workflow
    if current_status == 'pending_tutor':
        if workflow not in ['tutor_only', 'tutor_hod', 'tutor_hod_principal']:
            return error_response('This form does not require tutor approval')
        try:
            tutor = Teacher.objects.get(role='tutor', course_id=submission.student.course_id, is_active=True)
            ...
            return error_response('Multiple tutors found for course. Please contact admin.')
    
    elif current_status == 'pending_hod':
        if workflow not in ['hod_only', 'tutor_hod', 'tutor_hod_principal']:
            return error_response('This form does not require HOD approval')
        try:
            hod = Teacher.objects.get(role='hod', department_id=submission.student.department_id, is_active=True)
            ...
            return error_response('Multiple HODs found for department. Please contact admin.')
    
    elif current_status == 'pending_principal':
        if workflow != 'tutor_hod_principal':
            return error_response('This form does not require principal approval')
        try:
            principal = Teacher.objects.filter(role='principal').first()
            if not principal or request.user.id != principal.id:
    ...
    # Update submission based on current status
    if current_status == 'pending_tutor':
    ...
    # Set next status based on workflow
    submission.status = submission.get_next_status(current_status, action)
...
def update_form(request, form_id):
    ...
    if 'approval_workflow' in request.data:
        form.approval_workflow = request.data['approval_workflow']
    if 'department' in request.data:
        form.department_id = request.data['department']
