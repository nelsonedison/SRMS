...
class Teacher(models.Model):
    ...
    def clean(self):
        ...
        if self.role == 'hod' and self.is_active and self.department_id:
            existing_hod = Teacher.objects.filter(
                role='hod', department_id=self.department_id, is_active=True
            ).exclude(pk=self.pk)
            if existing_hod.exists():
                raise ValidationError("Only one active HOD per department is allowed.")
        
        # Only one active tutor per course
        if self.role == 'tutor' and self.is_active and self.course_id:
            existing_tutor = Teacher.objects.filter(
                role='tutor', course_id=self.course_id, is_active=True
            ).exclude(pk=self.pk)
            if existing_tutor.exists():
                raise ValidationError("Only one active tutor per course is allowed.")
