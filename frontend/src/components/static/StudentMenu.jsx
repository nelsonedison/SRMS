...
const StudentMenu = () => {
  ...
  const menuItems = [
    { path: '/student', icon: <BarChart2 size={18} />, label: 'Dashboard', color: 'from-blue-500 to-cyan-500' },
    { path: '/student/forms', icon: <FileText size={18} />, label: 'Available Forms', color: 'from-green-500 to-emerald-500' },
    { path: '/student/submissions', icon: <Eye size={18} />, label: 'My Submissions', color: 'from-purple-500 to-violet-500' },
    { path: '/student/profile', icon: <User size={18} />, label: 'Profile', color: 'from-pink-500 to-rose-500' },
    // { path: '/student/settings', icon: <Settings size={18} />, label: 'Settings', color: 'from-gray-500 to-slate-500' }
  ];
    ...
    <aside className={`h-screen bg-white/90 backdrop-blur-md border-r border-white/20 shadow-xl transition-all duration-300 ${collapsed ? 'w-20' : 'w-80'} flex flex-col`}>
      ...
      <div className={`${collapsed ? 'p-2' : 'p-4'} border-t border-gray-100/50`}>
        <div className={`flex items-center ${collapsed ? 'flex-col gap-2 p-2' : 'gap-3 p-3'} rounded-xl bg-gradient-to-r from-gray-50 to-gray-100`}>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-r from-green-600 to-emerald-600 text-white font-semibold flex items-center justify-center shadow-lg">
            {user?.name ? user.name.charAt(0).toUpperCase() : 'S'}
          </div>
          {!collapsed && (
            <div className="flex-1">
              <div className="text-sm font-semibold text-gray-900">{user?.name || 'Student User'}</div>
